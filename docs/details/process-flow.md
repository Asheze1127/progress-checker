...
| 主要コンポーネント | Slack、Webhook API、Idempotency Store、SQS、Worker、PostgreSQL、LLM、GitHub API |
...
flowchart LR
    ...
    R -->|question| QQ[SQS question へ送信]
...
flowchart LR
    ...
    QQ[SQS question] --> QW[Question Worker]
...
sequenceDiagram
    ...
    User->>Slack: /question 入力
    Slack->>API: command payload
    API->>API: 署名検証
    API->>IDS: idempotency_key 登録確認
    API->>SQSQ: question message 送信
    API-->>Slack: 200 OK（3秒以内）
```
